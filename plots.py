...
class PlotPanel:
    ...
    def _build(self, parent):
        ...
        self.thresh_ln = self.ax_score.axhline(y=config.THRESHOLD_HIGH, color='#FFA500', lw=1.0, ls='--', label='Threshold')
        self.ax_score.legend(loc='upper right', fontsize=7, facecolor='#111111', edgecolor='#333333')
    ...
    def push(self, t, rssi, ping, score, baseline=None, stats=None):
        ...
        if stats is not None:
            ...
            std_ping = stats.get('ping_std', 0) if 'ping_std' in stats else (abs(ping - (self.ping_buf[-2] if len(self.ping_buf) > 1 else ping)) / 10.0)
            sc = stats.get('score', 0)
            row = [std_rssi, std_ping, sc]
            ...
            if len(self.heat_rows) > 5:
                ...
                tick_labels = [f'{int(t_min)}s', f'{int(t_min + (t_max-t_min)/2)}s', f'{int(t_max)}s']
                self.ax_heat.set_xticks([0, cols//2, cols-1] if cols > 3 else [0, cols//2])
                self.ax_heat.set_xticklabels(tick_labels[:3])
